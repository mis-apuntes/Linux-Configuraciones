...
> Técnicamente Linux es el kernel y no el Sistema Operativo.
...
Esto es un mecanismo de seguridad que evita que programas maliciosos hagan uso de los recursos de la máquina directamente, O que se ejecuten tareas que el usuario no quiere que sea hecho.

- Los *procesos de usuario* se ejecutan en ***user mode***, en este modo los procesos tiene acceso restringido y limitado a la memoria y CPU. Hacen uso de *llamadas al sistema* para accederlos.

  > **Recordar**: Los procesos de usuario residen en el espacio de usuario
...
Cuando un proceso de usuario requiere una tarea que solo el *kernel* lo puede hacer, hace uso de las ***llamadas al sistema***, y posteriormente devolverá un resultado. Por ejemplo abrir, leer o escribir en un fichero. 
...
El kernel de Linux es un archivo imagen comprimido ubicado en `/boot/vmlinuz-<version>`.
...
El kernel de Linux está compuesto por miles de módulos de los cuales solo algunos se cargan en el arranque del sistema y otros se cargarán cuando sea necesario.

1. Los módulos que **forman parte de la imagen del *kernel*** (`/boot/vmlinuz-<version>`), son módulos "esenciales" que tienen un uso frecuente, estos son los que se cargan junto al arranque del sistema operativo, se denominan ***módulos estáticos***. Por ejemplo: <!--EL MODULO DEL SISTEMA DE ARCHIVO O INTERFAZ DE RED (BUSCAR SUS NOMBRES)-->

2. Los **módulos enlazados dinámicamente** (también conocidos como LKM), son módulos que se cargan solo cuando son requeridos. por ejemplo: <!--BUSCAR EJEMPLO-->
...
Muchas distribuciones proporcionan sus propios kernels de mantenimiento de largo plazo (Longterm) que por los general son soportados por sus propios desarrolladores.

> **Debian** proporciona su kernel, en sus diferentes versiones https://tracker.debian.org/linux , y el repositorio Git es https://salsa.debian.org/kernel-team/linux

### Código fuente del Kernel de Linux

El sitio oficial para descargar el código fuente es: www.kernel.org

Otra opción es usar el comando `wget` para descargar el código fuente. (Ref. [this](https://kernel-team.pages.debian.net/kernel-handbook/ch-common-tasks.html#s-kernel-org-package))
...
>Complementar [Rebuilding official Debian kernel packages](https://kernel-team.pages.debian.net/kernel-handbook/ch-common-tasks.html#s-common-official)
